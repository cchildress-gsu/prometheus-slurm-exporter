...
func FairShareData() []byte {
	cmd := exec.Command("sshare", "-n", "-P", "-o", "account,fairshare,normusage")
	...
	for _, line := range lines {
		if ! strings.HasPrefix(line, "  ") {
			if strings.Contains(line, "|") {
				...
				if !key {
					accounts[account] = &FairShareMetrics{0, 0}
				}
				fairshare, _ := strconv.ParseFloat(line_split[1], 64)
				normusage, _ := strconv.ParseFloat(line_split[2], 64)
...
func NewFairShareCollector() *FairShareCollector {
	...
	return &FairShareCollector{
		...
		normusage: prometheus.NewDesc("slurm_account_normusage", "Normalized usage for account", labels, nil),
	}
}
